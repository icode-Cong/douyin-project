...
func (*UserDao) AddFollowerCount(userId int64, cnt int32) {
	err := SqlSession.Model(User{}).Where("user_id=?", userId).Update("follower_count", gorm.Expr("follower_count+?", cnt)).Error
	if err != nil {
		//log.Error(err)
	}
}
...
func (*UserDao) AddFollowingCount(userId int64, count int32) {
	err := SqlSession.Model(User{}).Where("user_id=?", userId).Update("following_count", gorm.Expr("following_count+?", count)).Error
	if err != nil {
		//log.Error(err)
	}
}
...
func (*UserDao) GetUsersByIds(userIds []int64) ([]*User, error) {
	...
	err := SqlSession.Where("user_id IN (?)", userIds).Find(&users).Error
	if err != nil {
...
func (*UserDao) AddWorkCount(userId int64, count int32) {
	err := SqlSession.Model(User{}).Where("user_id=?", userId).Update("work_count", gorm.Expr("work_count+?", count)).Error
	if err != nil {
		//log.Error(err)
	}
}
...
func (*UserDao) AddFavoriteCount(userId int64, count int32) {
	err := SqlSession.Model(User{}).Where("user_id=?", userId).Update("favorite_count", gorm.Expr("favorite_count+?", count)).Error
	if err != nil {
		//log.Error(err)
	}
}
...
func (*UserDao) AddTotalFavorited(userId int64, count int32) {
	err := SqlSession.Model(User{}).Where("user_id=?", userId).Update("total_favorited", gorm.Expr("total_favorited+?", count)).Error
	if err != nil {
		//log.Error(err)
	}
}
